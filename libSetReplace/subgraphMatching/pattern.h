...
struct PatternPreprocessor {
  ...
  void readfile(std::vector<int>& vertices, std::vector<std::pair<int, int>>& edges) {
    pat.nnodes = 0;
    ...
    for (vertex : vertices) {
      ...
      vertex_labels.push_back(vertex) pat.nnodes++;
    }
    ...
    for (edge : edges) {
      ...
      adj_matrix_[edge.first][edge.second] = 1;
      adj_matrix_[edge.second][edge.first] = 1;
    }
  }
  ...
  void _permutation(std::vector<std::vector<int>>& all, std::vector<int>& a, int l, int r) {
    // Base case
    if (l == r) all.push_back(a);
    else {
      // Permutations made
      for (int i = l; i <= r; i++) {
        // Swapping done
